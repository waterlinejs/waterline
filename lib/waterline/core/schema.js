  ...
  for (var key in value) {
    switch (key) {
      ...
      case 'model':
        ...
        var pk = _.findWhere(attrs, { primaryKey: true })

        if (pk.type) attr.type = pk.type.toLowerCase();
        if (pk.dbType) attr.dbType = pk.dbType.toLowerCase();
        attr.model = value.model.toLowerCase();
